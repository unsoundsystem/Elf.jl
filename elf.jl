...
println(ehdr)
println(IS_ELF(ehdr))
println(ehdr.e_ident[EI_CLASS] == ELFCLASS64)
println(ehdr.e_ident[EI_DATA] == ELFDATA2MSB)
println(ehdr.e_shoff, "\n", ehdr.e_shentsize, "\n", ehdr.e_shstrndx)
shdr = Elf64_Shdr(file[65:ehdr.e_shoff+ehdr.e_shentsize*ehdr.e_shstrndx])
println("sizeof(ehdr) = ", sizeof(ehdr))
println("sizeof(shdr) = ", sizeof(shdr))
println(shdr)
# println(shdr)
